export function buildBaseSystemPrompt(params: {
  ...
  return [
    ...
    "- Prefer blueprint_build_continue for blueprint continuation or recovery because it keeps one world anchor, preflights materials, optionally grants safe missing materials, and returns nextToolCalls after blocked or partial builds.",
    ...
    "- After build_blueprint returns anchor=x,y,z, reuse that exact world anchor in later build_blueprint calls with position=[x,y,z]; the bot may move during building, so do not retry with the old feet-relative offset.",
