...
export async function probeNextCraftAction(opts: {
  ...
  const recipeHint = (() => {
    ...
    if (recipeNeedsRows > 0 && recipeNeedsCols > 0 && (recipeNeedsRows > gridRows || recipeNeedsCols > gridCols)) {
      lines.push(`GRID TOO SMALL: this recipe needs a ${recipeNeedsRows}x${recipeNeedsCols} grid but the current GUI only offers ${gridRows}x${gridCols}. You are the CRAFT sub-agent and only handle in-GUI placement. Escalate to the supervisor with structured reason so it can dispatch the right world sub-agent next:`);
      lines.push(`  emit: {"action":"fallback_manual","reason":"BLOCKED: original task = '${opts.taskText}' (crafting ${recipeInfo.target}) cannot proceed in this ${gridRows}x${gridCols} GUI. PREREQUISITE: a crafting_table 3x3 GUI must be open. Decision tree for the supervisor: (1) if a crafting_table is already visible in front of the player in the world, dispatch the world sub-agent to face it and 'use' it; (2) else if the player has a crafting_table item in hotbar/inv, dispatch placing sub-agent to place + use it; (3) else if the player has at least 4 planks, dispatch a NEW CRAFT subgoal to craft a crafting_table first (recipe: 4 planks in 2x2 -> crafting_table), THEN come back to (2); (4) else dispatch world sub-agent to find planks/wood. After the 3x3 GUI is open, RE-DISPATCH the craft sub-agent to resume with the SAME original task '${opts.taskText}'."}`);
      lines.push(`Do NOT attempt 3x3 placements in this 2x2 GUI.`);
    }
    if (gridCols > 0) {
    ...
    if (gridCols > 0) {
      ...
      if (plan.length > 0) {
        ...
        let nextStep = -1;
        const stepStates: Array<{ done: boolean; cell: typeof craftSlots[0] | undefined }> = plan.map((p, i) => {
          const cellIdx = (p.row - 1) * gridCols + (p.col - 1);
          const cell = craftSlots[cellIdx];
          let done = false;
          if (cell && opts.knownSlots) {
            const k = opts.knownSlots.find((ks) => ks.index === cell.index);
            if (k && ingMatches(k.item, p.ingredient)) done = true;
          }
          if (!done && nextStep < 0) nextStep = i;
          return { done, cell };
        });
  ...
  const promptText = [
    ...
    `  {"action": "done", "reason": "...", "subTask": "..."} -- emit AS SOON AS Known slot contents shows the recipe target item${recipeInfo ? ` (${recipeInfo.target})` : ""} present in any regular hotbar/main_inv slot (i.e. NOT one of the craft grid input/result slots). Do NOT wait for the result slot to clear or for additional place/take cycles — once the target is in regular inventory the task is complete. Extra cycles risk losing materials and hallucinating further crafts.`,
    "",
