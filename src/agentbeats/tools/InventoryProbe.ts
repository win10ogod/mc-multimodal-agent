...
export async function probeNextCraftAction(opts: {
  ...
  const recipeHint = (() => {
    ...
    if (recipeNeedsRows > 0 && recipeNeedsCols > 0 && (recipeNeedsRows > gridRows || recipeNeedsCols > gridCols)) {
      lines.push(`GRID TOO SMALL: this recipe needs a ${recipeNeedsRows}x${recipeNeedsCols} grid but the current GUI only offers ${gridRows}x${gridCols}. You are the CRAFT sub-agent and only handle in-GUI placement. Escalate to the supervisor: emit fallback_manual with reason="need to face a crafting_table and 'use' it to open the 3x3 GUI; if a crafting_table is already visible in the world view in front of the agent, simply 'use' it; otherwise place / approach one first". The supervisor will dispatch a world sub-agent to handle facing/placing/using and return control here once the 3x3 GUI is open. Do NOT attempt placements in this 2x2 GUI.`);
    }
    if (gridCols > 0) {
