...
- For ANY GUI interaction (inventory, crafting, smelting, brewing, chest, anvil, enchanting, villager trade, etc.): when the cursor is carrying an item, click only slots that are EITHER (a) visually empty OR (b) already contain the SAME item as the one the cursor is holding (placing onto a same-item slot stacks the two piles together — that is fine). Clicking a slot that contains a DIFFERENT item triggers a swap and corrupts state. If you must deposit into a slot occupied by a different item, do this 3-step swap-safe sequence (cursor must be empty between any two pickups): (1) place your currently-held item into an empty side slot to park it; (2) pick up the blocking item and place it in another empty slot; (3) pick up the parked item from step 1 and place it into the now-empty target slot.
