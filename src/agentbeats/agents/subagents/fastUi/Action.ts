...
- verify_items_visible: emit verify_slots on up to 3 candidates from items[] OR the explicit slots[] passed in. Emit done when every requested item has been checked — i.e. each item is EITHER named in Known (present) OR you have already OCR'd the plausible candidate slots without finding it (confirmed absent). Both outcomes count as "checked"; absence IS a valid answer for the planner. Do NOT keep emitting verify_slots after every item has been resolved either way.
